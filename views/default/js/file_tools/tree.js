...
define(function(require) {
	...
	function get_selected_tree_folder_id() {
		...
		var tree = $.tree.reference($("#file_tools_list_tree"));
	...
	$(function() {
		...
		$('select[name="file_sort"], select[name="file_sort_direction"]').change(function() {
			...
			var folder_url = elgg.get_site_url() + "file_tools/list/" + elgg.page_owner.guid + folder_guid
				+ ' = ' + get_selected_tree_folder_id() + "&search_viewtype&sort_by="
				+ $('select[name="file_sort"]').val() + "&direction=" + $('select[name="file_sort_direction"]').val();

			$("#file_tools_list_files_container").load(folder_url);
		});

		$('#file_tools_action_bulk_download').click(function() {
